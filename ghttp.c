...
static void reset_callback(g_connection_pt p);
...
static int do_pair_recv_header(g_connection_pt conn);
...
static g_connection_pt init_noused(g_connection_pt conns, int socket) {
    ...
    if (NULL != p->rsp_header) {
        cJSON_Delete(p->rsp_header);
        p->rsp_header = NULL;
    }
    p->head_length = 0;
    p->content_length = 0;
...
static void clear_conn(g_connection_pt conn) {
    ...
    if (NULL != conn->rsp_header) {
        cJSON_Delete(conn->rsp_header);
        conn->rsp_header = NULL;
    }
    conn->head_length = 0;
...
static void reset_callback(g_connection_pt p) {
    switch (p->conn_type) {
        ...
        case conn_type_pair:
            p->do_send = NULL;
            p->do_recv = do_pair_recv_header;
            p->do_close = do_pair_close;
            p->do_timer = NULL;
            p->tc_done = NULL;
            break;
        case conn_type_nouse:
        default:
            p->do_send = NULL;
            p->do_recv = NULL;
            p->do_close = NULL;
...
static int do_pair_recv_header(g_connection_pt conn) {
    ...
    if (ret < 0) {
        ...
        // peer close
        // need re-fork() another tc_worker
        conn->do_close(conn);
        return -2;
    }
    else {
        ...
        p = strstr(conn->recv_buf, "\r\n\r\n");
        if (p) {
            more = conn->bytes_recved - (p + 4 - conn->recv_buf);
            *p = '\0';
            conn->header = cJSON_Parse(conn->recv_buf);
            if (NULL == conn->header) {
                Error("%s(%d): parse recv_buf failed\n", __FUNCTION__, __LINE__);
                conn->do_close(conn);
            }
            cj = cJSON_GetObjectItem_EX(conn->header, "body_len");
            if (NULL == cj) {
                Error("%s(%d): get `body_len` failed\n", __FUNCTION__, __LINE__);
                conn->do_close(conn);
                return -3;
            }
            rsp_len = cj->valueint;
            Info("%s(%d): rsp_len=%d\n", __FUNCTION__, __LINE__, rsp_len);
            conn->body_recv_buf = (char*)malloc(rsp_len);
            if (NULL == conn->body_recv_buf) {
                conn->do_close(conn);
                return -4;
            }
...
finish_recv:
            ...
            if (NULL == cj) {
                Error("%s(%d): get session_id failed\n", __FUNCTION__, __LINE__);
                clear_conn(conn);
                return -3;
            }
            session_id = cj->valueint;
            cj = cJSON_GetObjectItem_EX(conn->header, "sockfd");
            if (NULL == cj) {
                Error("%s(%d): get session_id failed\n", __FUNCTION__, __LINE__);
                clear_conn(conn);
                return -3;
            ...
            sockfd = cj->valueint;
            client_conn = take_conn(global.conns, sockfd, session_id);
            if (NULL == client_conn) {
                Error("%s(%d): client is changed, drop this rsp. sockfd=%d, session_id=%d\n", 
                        __FUNCTION__, __LINE__, sockfd, session_id);
            }
            else {
                if (NULL == client_conn->tc_done) {
                    Error("%s(%d): tc_done not set, drop this rsp. sockfd=%d, session_id=%d\n", 
                            __FUNCTION__, __LINE__, sockfd, session_id);
                }
                else {
                    client_conn->tc_done(client_conn, conn->body_recv_buf, conn->content_length);
                    conn->body_recv_buf = NULL;
                }
            }
            clear_conn(conn);
...
static int business_worker(void) {
    ...
    for (i=0; i<global.proc_num; ++i) {
        ...
        fcntl(pair_conn->sockfd, F_SETFD, FD_CLOEXEC);
        if (ioctl(pair_conn->sockfd, FIONBIO, &nb)) {
            Error("%s(%d): ioctl(%d, FIONBIO) failed. error(%d): %s\n",
                    __FUNCTION__, __LINE__, pair_conn->sockfd, errno, strerror(errno));
            close(pair_conn->sockfd);
            return -5;
        }
...
static char* job_all_task(char* m) {
    MYSQL* db = NULL;
    MYSQL_RES* res = NULL;
    int row_c = 0;
    MYSQL_ROW row;
...
static char* job_set_task(char* m) {
    MYSQL* db = NULL;
    int ret_code = 0;
    ...
    if (0 != mysql_query(db, m)) {
        ...
        cJSON_AddNumberToObject(root, "ret_code", ret_code);
        cJSON_AddStringToObject(root, "ret_msg", "success");
        cJSON_AddNumberToObject(root, "task_id", mysql_insert_id(db));
    }
    out = cJSON_PrintUnformatted(root);
    cJSON_Delete(root);
    return out;
...
static char* job_del_task(char* m) {
    MYSQL* db = NULL;
    int ret_code = 0;
...
static int tc_worker(int sockfd) {
    ...
    int left = 0;
    int sent = 0;
    cJSON* root = NULL, *cj = NULL, *rsp = NULL;
    char* header = NULL;
    char* body = NULL;
    int rlen = 0;
    char* r = NULL;
    ...
    while(1) {
        if (-1 == msgrcv(global.mqid, (void *)&m, sizeof(m), 0, 0)) {
            if (ENOMSG == errno) {
                usleep(10);
                continue;
            }
            ...
            root = cJSON_Parse(m.mstring);
            if (NULL == root) {
                Error("%s(%d): cJSON_Parse(%s) failed, error(%d): %s\n", 
                        __FUNCTION__, __LINE__, m.mstring, errno, strerror(errno));
                continue;
            }
            switch (m.mtype) {
                case MSG_ALL_TASK:
                    // all_task
                    cj = cJSON_GetObjectItem_EX(root, "sql");
                    if (NULL == cj) {
                        Error("%s(%d): cJSON_GetObjectItem_EX(%s) failed, error(%d): %s\n", 
                                __FUNCTION__, __LINE__, "sql", errno, strerror(errno));
                    }
                    else {
                        body = job_all_task(cj->valuestring);
                    }
                    break;
                case MSG_SET_TASK:
                    // set_task
                    ...
                    if (NULL == cj) {
                        ...
                        body = job_set_task(cj->valuestring);
                    }
                    break;
                case MSG_DEL_TASK:
                    // del_task
                    ...
                    if (NULL == cj) {
                        ...
                        body = job_del_task(cj->valuestring);
                    }
                    break;
                default:
                    Error("%s(%d): unknown message, type=%d\n", __FUNCTION__, __LINE__, m.mtype);
                    break;
            }
            if (NULL == body) {
                continue;
            ...
            cj = cJSON_GetObjectItem_EX(root, "session_id");
            cJSON_AddNumberToObject(rsp, "session_id", cj->valueint);
            cj = cJSON_GetObjectItem_EX(root, "sockfd");
            cJSON_AddNumberToObject(rsp, "sockfd", cj->valueint);
            cJSON_AddNumberToObject(rsp, "body_len", strlen(body));

            header = cJSON_PrintUnformatted(rsp);
            cJSON_Delete(rsp);

            rlen = strlen(header) + strlen(body) + 4 + 1;
            r = (char*)malloc(rlen); // `\r\n\r\n\0`
            if (NULL == r) {
                Error("%s(%d): malloc(%d) failed\n", __FUNCTION__, __LINE__, rlen);
                continue;
            }
            safe_snprintf(r, rlen, "%s\r\n\r\n%s", header, body);
            free(header);
            header = NULL;
            free(body);
            body = NULL;
            ...
            left = rlen - 1;
            while(left) {
                ret = send(sockfd, r+sent, left, 0);
                Info("%s(%d): sent %d bytes\n", __FUNCTION__, __LINE__, ret);
                if (ret < 0) {
                    if (EAGAIN == errno || EINTR == errno) {
                        Info("%s(%d): send(%d,%u) not ready\n", __FUNCTION__, __LINE__, sockfd, left);
                        return 0;
                    }
                    Error("%s(%d): send(%d,%u) failed. error(%d): %s\n", 
                            __FUNCTION__, __LINE__, sockfd, left, errno, strerror(errno));
                    close(sockfd);
                    break;
                }
                else {
...
static int on_all_task_tc_done(g_connection_pt conn, char* data, int len) {
    cJSON* cj = NULL;
    char* out = NULL;
    int out_len = 0;
    char* jsonp = NULL;
    ...
    if (jsonp) {
        ...
        lucky_size = conn->content_length + 1 + LUCKY_NUM;
        jsonp_out = (char*)malloc(lucky_size);
        cur = jsonp_out;
        cur = xcpymem(cur, jsonp, jsonp_len);
        cur = xcpymem(cur, "(", 1);
        cur = xcpymem(cur, out, out_len);
        cur = xcpymem(cur, ")", 1);
        // safe_snprintf(jsonp_out, lucky_size, "%s(%s)", jsonp, out);
...
static int on_all_task(g_connection_pt conn) {
    ...
    root = cJSON_CreateObject();
    cJSON_AddNumberToObject(root, "session_id", conn->session_id);
    cJSON_AddNumberToObject(root, "sockfd", conn->sockfd);
    // create sql
    safe_snprintf(sql, sizeof(sql)-1, "select * from timer_tasks");
    cJSON_AddStringToObject(root, "sql", sql);
    ...
    // msgsnd
    m.mtype = MSG_ALL_TASK;
    safe_memcpy_0(m.mstring, sizeof(m.mstring), out, strlen(out));
    Info("%s(%d): %s\n", __FUNCTION__, __LINE__, m.mstring);
    if (-1 == msgsnd(global.mqid, &m, strlen(m.mstring)+sizeof(m.mtype), IPC_NOWAIT)) {
        if (EAGAIN == errno) {
    ...
    free(out);
    out = NULL;
    conn->tc_done = on_all_task_tc_done;
    return 0;
}

static int on_set_task_tc_done(g_connection_pt conn, char* data, int len) {
    char first_line[256];
...
static int on_set_task(g_connection_pt conn) {
    ...
    free(out);
    out = NULL;
    conn->tc_done = on_set_task_tc_done;
    return 0;
}

static int on_del_task_tc_done(g_connection_pt conn, char* data, int len) {
    char first_line[256];
...
static int on_del_task(g_connection_pt conn) {
    ...
    free(out);
    out = NULL;
    conn->tc_done = on_del_task_tc_done;
    return 0;
}
